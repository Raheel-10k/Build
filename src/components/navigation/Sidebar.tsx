...
export default function Sidebar() {
  ...
  return (
    <div className="h-screen w-64 bg-[#1A1A1D] border-r fixed left-0 top-0 p-4">
          ...
          <ul className="space-y-2">
            {navItems.map((item) => (
              <li key={item.label}>
                <Link
                  to={item.path}
                  className="flex items-center space-x-3 px-4 py-3 text-white rounded-lg hover:bg-indigo-50 hover:text-indigo-600 transition-colors"
                >
